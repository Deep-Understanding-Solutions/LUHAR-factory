...
source_id = "slobodnyvyber"

label = 0

sitemaps = (
    "https://slobodnyvyber.sk/sitemap-1.xml",
    "https://slobodnyvyber.sk/sitemap-2.xml",
)
db_keys = (
...
with shelve.open('counter') as db:
    ...
    while db[session_selector_key] != len(db_keys):
        ...
        for article_decrement in range(total_articles - parsed_articles):
            ...
            try:
                content = soup.find('div', attrs={'class': 'post-content'}).find_all('p')
                content = list(map(
                    lambda paragraph: ""
                    .join(
                        paragraph
            ...
            try:
                category = ""\
                    .join(
                        soup.find('div', attrs={'class': 'post-meta'}).find("a", attrs={'rel': 'tag'})
                            .get_text()
                            .strip()
                            .replace("\xa0", " ")
                            .replace("\xad", "-")
